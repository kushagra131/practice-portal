...
public class Search {
    ...
    private static int interpolationSearch(int[] array, int element) {
        ...
        while (left <= right && element >= array[left] && element <= array[right]) {
            ...
            if (array[pos] < element) left = pos + 1;
    ...
    private static int exponentialSearch(int[] array, int element) {
        ...
        //increase i = i*2
        while (i < array.length && array[i] <= element) i = i * 2;
    ...
    private static int ternarySearch(int[] array, int left, int right, int element) {
        if (left <= right) {
            ...
            if (array[mid1] == element) return mid1;
            if (array[mid2] == element) return mid2;
            if (array[mid1] > element) return ternarySearch(array, left, mid1 - 1, element);
            if (array[mid2] > element) return ternarySearch(array, mid1 + 1, mid2 - 1, element);
            else return ternarySearch(array, mid2 + 1, right, element);
        }
        return -1;
    }
