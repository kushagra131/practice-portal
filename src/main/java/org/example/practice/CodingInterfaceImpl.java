...
import org.example.practice.interfaces.CodingInterface;
...
//            System.out.println();
    ...
    public boolean isPrime(int number) {
        ...
        while (i <= number) {
            ...
            if (i == number && number % i == 0) count++;
            else if (i != number && number % i == 0) count++;
            i++;
        }
    ...
    public void fib(int n) {
        if (n < 2) System.out.println("Series cannot be printed!!");
        else {
            int fib1 = 0, fib2 = 1;
            int i = 1;
    ...
    public int maxConsecutiveOnes(int[] arr) {
        ...
        while (i < arr.length) {
            if (arr[start_ptr] != 1 && arr[end_ptr] != 1) {
                ...
                if (max_count <= prev_count) max_count = prev_count;
                curr_count = 0;
            }
        }
